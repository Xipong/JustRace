...
UPGRADE_BASE_COST_MULT = 0.08  # fraction of car price per part at level 0
...
# bonus applied after completing a full upgrade level
LEVEL_COMPLETION_BONUS = {"power": 0.01, "tire_grip": 0.01, "mass": -0.005}
...
TOTAL_PARTS_PER_LEVEL = PARTS_PER_CLASS + 1  # including custom package
...
def car_stats(p: Player, car_id: str) -> Dict[str, float]:
    ...
    progress = p.upgrades.get(car_id, UpgradeProgress())
    max_parts = UPGRADE_CLASSES.get(tier, 0) * TOTAL_PARTS_PER_LEVEL
    ...
    for idx, pid in enumerate(applied):
        level_idx = idx // TOTAL_PARTS_PER_LEVEL
    ...
    # apply completion bonus for each fully upgraded level
    completed = len(applied) // TOTAL_PARTS_PER_LEVEL
    for level_idx in range(completed):
        lvl_mult = UPGRADE_LEVEL_EFFECT_MULTS[min(level_idx, len(UPGRADE_LEVEL_EFFECT_MULTS) - 1)]
        power *= 1 + LEVEL_COMPLETION_BONUS["power"] * lvl_mult
        mass *= 1 + LEVEL_COMPLETION_BONUS["mass"] * lvl_mult
        grip *= 1 + LEVEL_COMPLETION_BONUS["tire_grip"] * lvl_mult
    return {
        "power": power,
        "mass": mass,
        "tire_grip": grip,
